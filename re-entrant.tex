...
\begin{table}
  ...
  \begin{tabular}{
    ...
    \midrule
       5 \si{\degree}  &  {$\infty$}  &  13.0048    &  {N/A} \\
      10 \si{\degree}  &  {$\infty$}  &  11.1065    &  {N/A} \\
      15 \si{\degree}  &  {$\infty$}  &   8.76736   &  {N/A} \\
      20 \si{\degree}  &  {$\infty$}  &   6.5604    &  {N/A} \\
      25 \si{\degree}  &  {$\infty$}  &   4.79678   &  {N/A} \\
...
Note that $\ell_\ultra = 0.01$~corresponds to the existing level of refinement
...
achieves an accuracy of just~$\SI{6}{\percent}$.
From Figure~X % TODO
we see that the dependence of the solution slope on the refinement
is restricted only to the immediate vicinity of the corner, $r < 0.003$;
the computed slope outside this tiny neighbourhood
is indeed consistent across all levels of additional refinement.
...
unlike corner slope,
the same corner height is consistently obtained
with or without additional refinement.

...
\begin{figure}
  \centredfigurecontent[width=0.56\textwidth]{%
    ...
    which touches the inner candidate boundary
...
\footnote{
  ...
  if we had chosen $A_\infl$ instead of the special value~$A_{\nat 0}$
  for the upper bound of the continuum~%
  (\ref{eq:bipolar-inner-candidate-existence-interval}).
}
so we have
\begin{equation}
...
\end{equation}
Therefore,
the temperature~(\ref{eq:bipolar-t-sharp-0-in-terms-of-r-sharp-0})
spans the interval
\[
  0
    <
...
complements \emph{perfectly} the analogous line-source result~%
...
Physically, the introduction of a negative line-source
has the effect of extending the temperature interval down to absolute zero.

...
for all~$0 < A \le A_{\nat 0}$
as in Figure~X\@: % TODO
an \term{outer candidate boundary}
by performing an upper-to-lower branch switch at~$(0, v_{\flat 0})$,
and an \term{inner candidate boundary}
by the same at~$(0, v_{\sharp 0})$,
so called because the inner candidate boundary
is always surrounded by the outer candidate boundary
(with the two boundaries becoming one at~$A = A_{\nat 0}$).

...
(whether it be a rounding, a simple truncation, or a combination of these),
...
(e.g.~small-scale corner rounding)
is subtle:
...
The simplest choice is to set~$\theta_\modi = 0$ everywhere,
so that the constructed modified boundaries are simply $T$-contours.
The comparison observation implies the following:
\begin{corollary*}
  ...
  (for a tracing contact angle of~$\gamma$).
  Then the height rise profile of~$T_\modi$ is bounded above by~$h_\modi$.
\end{corollary*}
In other words,
by truncating a wedge in the shape of a completely viable $T$-contour,
we will guarantee that the resulting height rise profile
is bounded above by that height of that $T$-contour.
...
\begin{figure}
  ...
  \begin{subfigure}[t]{0.5\textwidth}
    \centredfigurecontent[
      ...
      and the corresponding height rise profile~\figurestyle{solid}

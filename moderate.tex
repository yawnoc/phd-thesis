...
we test the numerical solver in a $10 \times 1$~rectangle,
...
\begin{table}
  ...
  \begin{tabular}{
    S[table-format=1.3]
    S[table-format=4]
    S[table-format=1.6, round-mode=figures, round-precision=5]
    S[table-format=+1.1e+1, round-mode=figures, scientific-notation=true]
  }
    \toprule
      {$\ell$}  &  {Mesh elements}  &  {Computed height}  &  {Relative error}
      \\
    \midrule
  ...
  \end{tabular}
  \caption{
    Numerical half-plane wall height for~$\gamma = \SI{1}{\degree}$,

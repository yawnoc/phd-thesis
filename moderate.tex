...
Only one critical terminal point~$(x_0, y_0)$ exists,
located at the intersection
between the terminal curve~(\ref{eq:contact-terminal curve})
and the line of symmetry~$y = 0$.
Explicitly we have~$y_0 = 0$,
...
so the critical terminal point~$(x_0, y_0)$ is of hyperbolic type.
Thus, as in the linearised Helmholtz case,
there are two smooth traced boundaries through~$(x_0, y_0)$.
...
\begin{align*}
  \squarebr*{P^2 - F^2}_{\xi=\vd\xi}
    ...
    &=
      \squarebr*{
        \sin^2\gamma \cdot \pder{(P^2)}{\xi}
          -
        \cos^2\gamma \cdot \pder{(Q^2)}{\xi}
      }_{\xi=0}
        \cdot
      \vd\xi
      + \order\roundbr*{\vd\xi^2}
...
Briefly ignoring the borderline case~$\alpha = \pi/2 - \gamma$,
the known solution near the corner takes on
the locally planar form~(\ref{eq:moderate-wedge-asymptotic-solution}),
or
\begin{equation}
  T \asy T_0 - \frac{x}{\sqrt{k^2 - 1}},
  \label{eq:moderate-asymptotic-solution}
\end{equation}
where $k$~is given by~(\ref{eq:wedge-constant-k}),
repeated here for convenience:
\begin{equation}
  k = \frac{\sin\alpha}{\cos\gamma}.
  \label{eq:moderate-wedge-constant-k}
\end{equation}
Using the asymptotic form~(\ref{eq:moderate-asymptotic-solution}),
...
depends only on the parameter~$k$, (\ref{eq:moderate-wedge-constant-k}).

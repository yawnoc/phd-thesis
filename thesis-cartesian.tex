...
\begin{align}
  \normalvec \dotp \scaleddel \scaled{T}
    ...
    \label{eq:radiation-boundary-condition-slab-scaled-with-groups} \\
  \scaled{T}
    &= \squarebr*{\frac{h_0 \lambda}{\tau}} \scaled{x}.
    \label{eq:laplace-solution-slab-scaled-with-groups}
\end{align}
...
so that the scaled boundary condition~%
  (\ref{eq:radiation-boundary-condition-slab-scaled-with-groups})
and known solution~(\ref{eq:laplace-solution-slab-scaled-with-groups})
become
\begin{align}
  \normalvec \dotp \del T &= -T^4,
    \label{eq:radiation-boundary-condition-slab-scaled} \\
  T &= x.
    \label{eq:laplace-solution-slab-scaled}
\end{align}

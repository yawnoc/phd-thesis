...
\begin{figure}
  \centredfigurecontent[width=\textwidth]{%
    ...
    Unphysical region~$T < 0$ (black shaded) and contours~$T = \const$ (grey)
...
the geometry of the physical domain~$T \ge 0$
...
In addition to only considering the physical domain~$T \ge 0$,
...
In the $B = 1$~case,
the viability function~(\ref{eq:cosine-viability-function}) reduces to
\begin{equation}
...
The terminal curve recedes to the right with increasing~$A$,
passing through the straight line~$x = \pi/2$ when~$A = 1$.

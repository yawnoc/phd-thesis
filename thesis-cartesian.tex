...
\begin{figure}
  \centredfigurecontent[width=0.6\textwidth]{%
    ...
    Spike formed by the pair of traced boundaries
...
For each choice of integration constant
the corresponding pair of traced boundaries makes a spike
(Figure~\ref{fig:plane-traced-boundary-spike}),
and each spike forms a cusp
\begin{equation}
  y = \const \mp \frac{x^5}{5} + \order \roundbr*{x^{13}}
  \label{eq:plane-traced-boundary-x-near-0}
\end{equation}
at~$x = 0$.
Near~$x = 1$ there holds
\begin{equation}
  y = \const \pm \sqrt{\frac{1 - x}{2}} + \order (1 - x)^{3/2},
  \label{eq:plane-traced-boundary-x-near-1}
\end{equation}
and the curves do indeed attach smoothly
onto the critical terminal curve~$x = 1$ as expected.

The half-width (along~$x = 1$)
of the spike in Figure~\ref{fig:plane-traced-boundary-spike} is
\begin{equation}
  \frac{1}{5}
    \cdot
...
\end{equation}
(TODO: probably need to have a plot with a bunch of traced boundaries.)

...
\begin{figure}
  \centredfigurecontent[width=\textwidth]{%
    ...
    Unphysical region~$T < 0$ (black shaded)
    for the known solution~(\ref{eq:cosine-scaled-laplace-solution}),
    as $B$~increases.
    Also shown are the $T$-contours (thin grey).
    In each plot, the left edge is~$x = 0$
    and the vertical line (thicker grey) is~$x = \pi/2$,
...
Non-viable regions within the unphysical region~$T < 0$
have been omitted because they are irrelevant,
leaving a single non-viable region of interest to the right.%
\footnote{
  \label{ftn:cosine_simple-origin-terminal}
  Not evident from Figure~\ref{fig:cosine_simple-physical-viable}
  is that the boundary of this single non-viable region
  only constitutes \emph{almost all} of the terminal curve.
  Since $(x, y) = (0, 0)$~is an isolated root of~$\Phi = 0$,
  the origin is in fact single-point subset of the terminal curve.
  In other words, the origin is a terminal point,
  though most peculiarly disconnected from the rest of the terminal curve.
}
As $A$~increases, this non-viable region recedes towards the right,
completely passing over the vertical line~$x = \pi/2$ when~$A = 1$.
...
\begin{figure}
  \centredfigurecontent[width=\textwidth]{%
    ...
    Non-viable domain~$\Phi < 0$ (grey shaded)
    ...
    the $T$-contours (thin grey),
    and the terminal curve~$\Phi = 0$ (black dashed).
    In each plot, the left edge is~$x = 0$
    and the vertical line (thicker grey) is~$x = \pi/2$,
    which is also the contour~$T = 1$.
  }
...
i.e.~$\Phi > 0$,
the two traced boundaries through it cross at a non-zero angle,
...
\begin{figure}
  \centredfigurecontent[width=0.35\textwidth]{%
    ...
    $T$-contours (thin grey) which intersect
    the terminal curve~$\Phi = 0$ (black dashed),
    which is the border of the non-viable domain~$\Phi < 0$ (grey shaded).
...
\footnote{
  ...
  (see footnote~p.~\pageref{ftn:cosine_simple-origin-terminal}).
  This peculiar instance is made more remarkable by the local $T$-contour,
  which looks locally like~$y = \pm x$ (whose tangent is indeterminate).
  One therefore wonders:
    are the local $T$-contour and the local portion of the terminal curve
    (which is just a point) \emph{tangential},
    and therefore, is the origin a \emph{critical} terminal point?
  The answer is yes, on account of there being two traced boundaries
  passing through the origin as if $y = 0$~were the common tangent.
  However, this pair of traced boundaries
...
a \term{candidate domain} is marked out by using

...
the dependence of the non-viable domain on~$A$.
Within the vertical strip~$0 \le x \le \pi/2$ of interest,
the non-viable domain consists of three separate regions;
two of these always lie in the unphysical region~$T < 0$ and are irrelevant.
Only the third non-viable region lies in the physical region~$T \ge 0$;
it recedes towards the right as $A$~increases,
with its boundary (the physical portion of the terminal curve)
...
by integrating the boundary tracing ODE~%
  (\ref{eq:cosine-tracing-ode-coordinate-parametrisation-x})
forward from starting points within the region both physical and viable.
The resulting curves (Figure~TODO)
...
TODO:
\begin{itemize}
  \item Patch traced boundaries to make radiation boundaries
  \item Join radiation boundaries with a portion of the line~$x = \pi/2$
        to make conduction--radiation domains
  \item Rule out almost all tracings since they are self-viewing
  \item Must pass through critical terminal point~$(x_0, 0)$
        in order to avoid non-convex spike
\end{itemize}

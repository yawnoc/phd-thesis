...
To avoid the non-convex spikes caused
by patching traced boundaries which cross at an angle,
the join must occur at a point along the terminal curve.
Furthermore this point must not be an ordinary terminal point,
since the two traced boundaries being patched
would form a cusp which has inconsistent boundary orientation.
Only the join must occur at a critical terminal point,

...
Defining
\begin{align}
  \scaled{T} &\ideq T / \Theta, \label{eq:plane-scaled-temperature} \\
  \scaled{x} &\ideq x / \lambda, \label{eq:plane-scaled-x} \\
  \scaled{y} &\ideq y / \lambda, \label{eq:plane-scaled-y}
\end{align}
and noting that
\begin{equation}
...
For brevity, drop all \scalingmarks{}
...
The bisection algorithm yields the root
\begin{equation}
  A_\infl = 0.79718,
...
and one can be completely confident
...
with translational symmetry in~$y$ has been produced.
By construction
the radiation condition~(\ref{eq:radiation-boundary-condition})

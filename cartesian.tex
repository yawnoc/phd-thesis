...
While the candidate domains for~$A < A_\infl$ are not convex,
...
in which the amount of self-viewing radiation is negligible,
...
The analysis of Appendix~X yields % TODO
\begin{equation}
  \savecontent{
    R \le R_+ =
      \frac{
...
\end{equation}
with the upper bound~$R_+$ falling below~$\SI{1}{\percent}$ at
\begin{equation}
  A = A_\prac = 0.43124.
  \label{eq:cosine-simple-traced-boundary-a-practical}
\end{equation}
Thus, the non-convex candidate domains are of practical use
on the interval
\begin{equation}
...
of the interval~(\ref{eq:cosine-simple-traced-boundary-convex-a-interval})
for convexity.

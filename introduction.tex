...
\begin{equation}
  \normalvec \dotp \frac{\del T}{\sqrt{1 + (\del T)^2}} = \cos\contang,
  \label{eq:contact-boundary-condition}
\end{equation}
which says that the liquid surface must meet the container walls
at a contact angle of~$\contang$
(Figure~X). % TODO
Assuming $\contang$~is constant,
it suffices to consider~$\contang < \pi/2$;
if~$\contang = \pi/2$ we simply have the solution~$T \ideq 0$,
and if~$\pi/2 < \contang \le \pi$
we have an equivalent problem in~$-T$
with an acute contact angle of~$\pi - \contang$.

...
\begin{equation}
  \del \dotp \frac{\del T}{\sqrt{1 + (\del T)^2}} =
    \roundbr*{\frac{\densit \gravit}{\surften}} T + \const,
  \label{eq:capillary}
\end{equation}
...
Chapter~1 of Finn~\cite{finn-1986-equilibrium-capillary-surfaces}
gives a derivation based on variational principles.
Here we only consider the case of downward gravity ($\gravit > 0$),
i.e.~$\densit \gravit / \surften$~positive.%
\footnote{
  Comprehensive surveys of capillary phenomena including the cases of
  zero and inverted gravity ($\densit \gravit / \surften \le 0$)
  may be found in the many reviews of Finn~\cite{
    finn-1974-capillarity-phenomena,
    finn-2002-eight-properties-capillary-surfaces,
    finn-2002-some-properties-capillary-surfaces
  }
  and in his book~\cite{finn-1986-equilibrium-capillary-surfaces}.
}
The constant in~(\ref{eq:capillary}) is determined by a volume constraint
(or by the height at infinity when the volume of liquid is infinite),
and may be removed by applying a suitable offset to~$T$.

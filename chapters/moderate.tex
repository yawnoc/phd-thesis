...
which enables \emph{multiple} rounding curves
to be produced by boundary tracing
...
\begin{figure}
  \newcommand*{\figuretrim}{8}
  \includegraphics[width=\textwidth, trim=0 -6 0 0]{helmholtz-legend}
  \begin{minipage}[t]{0.5\textwidth}
    \centredfigurecontent[width=0.7\textwidth, trim=0 {\figuretrim} 0 0]{%
      helmholtz-viable%
    }{
...
\begin{figure}
  \centredfigurecontent[width=0.63\textwidth]{%
    ...
    $U$-contours which intersect the terminal curve~$\Phi = 0$.
  }
\end{figure}
...
\begin{figure}
  \centredfigurecontent[width=0.48\textwidth, trim=0 4 0 8]{%
    half_plane-solution%
  }{
...
\pagebreak % (to avoid orphaned "rectangle" line)
...
default meshing algorithm do its work,
...
\begin{figure}
  ...
  \begin{subfigure}[t]{0.58\textwidth}
    \centredfigurecontent[trim=0 12 0 0]{half_plane-mesh-halves}{%
      Full mesh
    }
  \end{subfigure}
    \hfill
  \begin{subfigure}[t]{0.38\textwidth}
    \centredfigurecontent[trim=0 7 0 0]{half_plane-mesh-detail}{%
...
\begin{table}
  ...
  \caption{
    ...
    The theoretical wall height is~$h = 1.4018$.
  }
...
\begin{figure}
  ...
  \begin{subfigure}[t]{0.67\textwidth}
    \centredfigurecontent[trim=0 -4 0 -4]{wedge_acute-mesh-full}{%
      Full mesh
    }
  \end{subfigure}
    \hfill
  \begin{subfigure}[t]{0.27\textwidth}
    \centredfigurecontent[trim=0 {-0.68\textwidth} 0 0]{%
...
in that case both the corner height and the corner slope are unbounded,
...
that meets a vertical wall at a contact angle of~$\gamma$.
...
\footnote{
  ...
  indeed turn out to never be horizontal,
...
in the viable domain~(\ref{eq:moderate-viable-domain}),
...
\begin{figure}
  \newcommand*{\legendoffsetheight}{0.293\textwidth}
  \centering
  \includegraphics[width=0.4\textwidth, trim=0 6 0 0]{%
    wedge_acute-traced-boundaries%
  }
  \includegraphics[
    width=0.3\textwidth,
...
until we make a novel observation
(not made in~\cite{li-2017-thesis-rounding-capillary-wedge}):
...
\&~(\ref{eq:scaled-contact-boundary-condition}),
in a wedge with half-angle~$\alpha$
...
depends only on the parameter~$k = \sin\alpha / {\cos\gamma}$.
...
\begin{figure}
  \centredfigurecontent[width=0.4\textwidth, trim=0 8 0 0]{%
...
\begin{figure}
  \centredfigurecontent[width=0.4\textwidth, trim=0 6 0 0]{%
...
\begin{figure}
  ...
  \newcommand*{\subfiguretrim}{6}
  \begin{subfigure}[t]{\subfigurewidth}
    \centredfigurecontent[trim=0 {\subfiguretrim} 0 0]{%
...
\begin{enumerate}
  \item
    ...
    the bulging candidates would not qualify as roundings,
...
and their resulting height rises,
...
\begin{table}
  ...
  \end{tabular}
  \caption{
    Well-approximating radius~%
      (\ref{eq:moderate-well-approximating-rounding-radius})
    and the resulting height rise,
    for a collection of rounding curves
    all corresponding to the physically prescribed angles~%
    $\alpha = \SI{60}{\degree}$ and~$\gamma_\tr = \SI{75}{\degree}$.
    The original contact angles~$\gamma$ are listed for reference.
  }
...
\&~(\ref{eq:contact-boundary-condition-diffusion})
in circularly-rounded wedges of various rounding radii.
...
the trend of decreasing height rise with increasing rounding radius.
